...
const PANEL = 'rounded-[14px] border border-kaleo-earth/10 bg-white p-5 shadow-sm';
...
export default function AdminContentPageFresh() {
  const { authFetch } = useAdminAuth();
  const { confirm } = useAdminConfirm();

  // Hard-stabilize background while in admin to prevent dark repaint during native dialogs.
  useEffect(() => {
    const prevHtmlBg = document.documentElement.style.backgroundColor;
    const prevBodyBg = document.body.style.backgroundColor;
    document.documentElement.style.backgroundColor = '#f4f5f7';
  ...
  const [newMedia, setNewMedia] = useState<NewMediaState>({
    media_type: 'IMAGE',
    src_url: '',
    video_embed_url: '',
    title: '',
  ...
  const [activeSection, setActiveSection] = useState<'hero' | 'promo' | 'testimonials' | 'policies' | 'gallery'>('hero');
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const rawBanner = bannerData.banner ?? {
        enabled: 0,
        message: '',
        image_url: null,
        cta_label: '',
      ...
      setBanner({
        ...rawBanner,
        start_at: rawBanner.start_at,
        end_at: rawBanner.end_at,
      });
  ...
  if (loading) {
    return <p className="font-body text-sm text-kaleo-earth/60">Loading content modules...</p>;
  }

  const sectionBtn = (id: 'hero' | 'promo' | 'testimonials' | 'policies' | 'gallery') =>
    `rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-wider transition ${
      activeSection === id
        ? 'bg-kaleo-terracotta text-white'
        : 'border border-kaleo-earth/20 bg-white text-kaleo-earth hover:border-kaleo-terracotta'
    }`;
    ...
    <div className="flex min-h-full flex-col gap-5 pb-2">
      <section className={PANEL}>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button type="button" onClick={() => setActiveSection('hero')} className={sectionBtn('hero')}>Hero</button>
          <button type="button" onClick={() => setActiveSection('promo')} className={sectionBtn('promo')}>Promo</button>
          <button type="button" onClick={() => setActiveSection('testimonials')} className={sectionBtn('testimonials')}>Testimonials</button>
          <button type="button" onClick={() => setActiveSection('policies')} className={sectionBtn('policies')}>Policies</button>
          <button type="button" onClick={() => setActiveSection('gallery')} className={sectionBtn('gallery')}>Gallery</button>
        </div>
      </section>

      {/* HERO */}
      {activeSection === 'hero' ? <section className={PANEL}>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <label className="md:col-span-2">
            ...
            />
            <span className={FILE_PICKER_BUTTON}>{uploadingHero ? 'Uploading hero...' : 'Choose hero image'}</span>
          </label>
          ...
          {hero.background_image_url ? (
            <>
              ...
              <button
                type="button"
                onClick={() => setHero((p) => ({ ...p, background_image_url: null }))}
      ...
      {/* PROMO BANNER */}
      {activeSection === 'promo' ? <section className={PANEL}>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <label className="md:col-span-2">
            ...
            <span className={FILE_PICKER_BUTTON}>
              {uploadingBanner ? 'Uploading banner image...' : 'Choose banner image'}
            </span>
          </label>
          ...
          <div className="md:col-span-2 min-h-[132px] overflow-hidden rounded-lg border border-kaleo-earth/15 bg-kaleo-sand/40">
            {banner.image_url ? (
              <img
              ...
              <div className="flex h-32 items-center justify-center">
                <p className="text-xs uppercase tracking-wider text-kaleo-earth/45">No banner image selected</p>
              </div>
            )}
          </div>
          <button
            ...
            onClick={() => setBanner((p) => ({ ...p, image_url: null }))}
            className="md:col-span-2 rounded-[9px] border border-red-300 px-4 py-2 text-xs uppercase tracking-wider text-red-700 disabled:cursor-not-allowed disabled:opacity-40"
          >
            Remove banner image
          </button>
          ...
          <input
            ...
            value={toDateTimeInputValue(banner.start_at)}
            onChange={(e) => setBanner((p) => ({ ...p, start_at: fromDateTimeInputValue(e.target.value) }))}
            className="rounded-xl border border-kaleo-earth/20 px-3 py-2"
          />
          <input
            type="datetime-local"
            title="Promo banner end date and time"
            value={toDateTimeInputValue(banner.end_at)}
            onChange={(e) => setBanner((p) => ({ ...p, end_at: fromDateTimeInputValue(e.target.value) }))}
            className="rounded-xl border border-kaleo-earth/20 px-3 py-2"
          />
        </div>
      ...
      {/* TESTIMONIALS */}
      {activeSection === 'testimonials' ? <section className={PANEL}>
      ...
      {/* POLICIES */}
      {activeSection === 'policies' ? <section className={PANEL}>
      ...
      {/* GALLERY */}
      {activeSection === 'gallery' ? <section className={PANEL}>
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          <select
            ...
            onChange={(e) =>
              setNewMedia((p) => ({
                ...p,
                media_type: e.target.value as GalleryMediaType,
              }))
            }
            className="rounded-xl border border-kaleo-earth/20 px-3 py-2"
          >
          ...
          {newMedia.media_type === 'IMAGE' ? (
            <label className="md:col-span-2">
              ...
              />
              <span className={FILE_PICKER_BUTTON}>
                {uploadingGalleryFile ? 'Uploading image...' : 'Choose gallery image'}
              </span>
            </label>
          ) : (
            <label className="md:col-span-2">
              ...
              />
              <span className={FILE_PICKER_BUTTON}>
                {uploadingGalleryFile ? 'Uploading video...' : 'Choose gallery video'}
              </span>
            </label>
          )}
          ...
          {newMedia.media_type === 'IMAGE' && newMedia.src_url ? (
            <div className="md:col-span-2 overflow-hidden rounded-lg border border-kaleo-earth/15">
              <img src={toAbsoluteMediaUrl(newMedia.src_url)} alt="New gallery preview" className="h-32 w-full object-cover" />
            </div>
          ) : null}
          ...
          {newMedia.media_type === 'VIDEO' && newMedia.src_url ? (
            <div className="md:col-span-2 overflow-hidden rounded-lg border border-kaleo-earth/15 bg-black">
              <video src={toAbsoluteMediaUrl(newMedia.src_url)} controls className="h-40 w-full object-cover" />
            </div>
          ) : null}
        ...
        <div className="mt-4 space-y-3">
          {gallery.map((row) => (
            <div key={row.id} className="rounded-xl border border-kaleo-earth/10 p-3">
              ...
              {row.media_type === 'IMAGE' && row.src_url ? (
                <img src={toAbsoluteMediaUrl(row.src_url)} alt="Gallery item preview" className="mb-2 h-28 w-full rounded-lg object-cover" />
              ) : null}
              {row.media_type === 'VIDEO' && row.src_url ? (
                <video src={toAbsoluteMediaUrl(row.src_url)} controls className="mb-2 h-28 w-full rounded-lg object-cover" />
              ) : null}
              {row.media_type === 'VIDEO' && row.video_embed_url ? (
                <iframe
                  src={row.video_embed_url}
                  title="Gallery video embed preview"
